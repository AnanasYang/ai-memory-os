#!/usr/bin/env python3
"""
使用 OpenClaw agent 命令调用 feishu_doc 工具
"""
...
# 构建消息 - 请求 agent 使用 feishu_doc 工具
message = f"""请使用 feishu_doc 工具将以下内容追加到飞书文档：
...
# 将消息保存到文件
message_file = "/tmp/agent_message.txt"
...
print(f"\n执行命令: openclaw agent --channel feishu --message [内容] --timeout 120")

# 执行命令
result = subprocess.run(cmd, capture_output=True, text=True)

print(f"\n返回码: {result.returncode}")
print(f"标准输出:\n{result.stdout[:2000]}..." if len(result.stdout) > 2000 else f"标准输出:\n{result.stdout}")
print(f"标准错误:\n{result.stderr[:2000]}..." if len(result.stderr) > 2000 else f"标准错误:\n{result.stderr}")

if result.returncode == 0:
    print("\n✅ Agent 执行成功!")
else:
    print(f"\n❌ Agent 执行失败 (返回码: {result.returncode})")